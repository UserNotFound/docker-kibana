...
@test "It should install Kibana 6.4.0" {
...
@test "docker-kibana proxies with credentials to Elasticsearch 6.4" {
  web_log="${BATS_TEST_DIRNAME}/web.log"
  ( while echo "$HTTP_RESPONSE_HEAD" '{"version": {"number": "6.4.0"}}' | nc -l 456; do : ; done ) > "$web_log" &
